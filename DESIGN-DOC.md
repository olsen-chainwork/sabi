...
- **2026-04-18 (Day 2, AM):** State machine for the popover locked as a 4-mode enum (`idle → augmenting → reviewing → saved`), with Edit returning to `.idle` without clobbering the saved intent. `@Observable` singletons + MainActor isolation became the default pattern for UI-bound state (IntentStore, SourcesStore, PollingPrefs, SeenLog). Nonisolated enums stayed the default for async I/O types (BraveClient, AnthropicClient, DomainAllowlist, Retrieval, Ranker, AugmentPrompt).
- **2026-04-18 (Day 2, AM):** "Curation *is* the product taste" design principle moved from research note to locked decision. Allowlist is mandatory, user-editable, and starts from ~60 curated defaults. Delta storage (additions + disabled set, not full list) preserves user overrides across default-list updates in future versions. Paywall-light sources (Stratechery, HBR) stay out of defaults for the contest; users can add them manually.
- **2026-04-18 (Day 2, midday):** Scheduled polling locked as part of the core identity, not a cuttable extra. `NSBackgroundActivityScheduler` every ~4h while the app is running. Pings fire only on "genuinely new" top-5 hits (past-week freshness + seen-log dedupe). Silent ticks are a feature — the product promises quiet, not a constant feed. Default polling ON; users opt out if they'd rather pull than be pushed.
- **2026-04-18 (Day 2, midday):** Retrieval diversity locked as a quality floor. Pool must surface ≥5 distinct hosts in the candidate list; round-robin-by-host before `prefix(limit)` + ≤2-per-canonical-domain cap in the ranker. Locked because "single-domain monoculture" is a failure mode that silently degrades perceived quality — and it's invisible until you see a top-5 screenshot.
- **2026-04-19:** Icon direction locked: paw print (after a detour through a dog bone that read phallic at 22pt). Menu bar glyph is a template image (system-tinted); app icon is a caramel paw on a cream vertical-gradient squircle. Earthy, warm palette, no hard branding. Post-feature roadmap reframed into three non-slice streams: visual polish (slice 8), speed (slice 9), "make it cooler" brainstorm (slice 10). Shipping prep (landing page, pitch video, Handshake submission, signed DMG story) runs in parallel once polish lands.
